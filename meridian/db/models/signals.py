...
class SignalDefinition(Base):
    ...
    category: Mapped[SignalCategory] = mapped_column(
        Enum(SignalCategory), nullable=False
    )
...
class SignalLog(Base):
    ...
    computed_output: Mapped[dict[str, Any]] = mapped_column(
        JSON, nullable=False, default=dict
    )
    ...
    signal: Mapped[SignalDefinition] = relationship(
        "SignalDefinition", back_populates="logs"
    )
